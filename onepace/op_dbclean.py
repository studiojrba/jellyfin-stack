...
SERIES = os.environ.get("ONEPACE_SERIES_ID")
if not SERIES:
    row = c.execute("SELECT Id FROM BaseItems WHERE Path LIKE '%/One Pace' AND Type LIKE '%Series'").fetchone()
    if not row:
        row = c.execute("SELECT Id FROM BaseItems WHERE Path LIKE '%/One Pace'").fetchone()
    if not row:
        raise SystemExit("Could not find the One Pace series in jellyfin.db (set ONEPACE_SERIES_ID to override)")
    SERIES = row[0]
print("One Pace series id:", SERIES)
