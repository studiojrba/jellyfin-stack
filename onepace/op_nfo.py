...
Race with Jellyfin's own scan schedule: if Jellyfin scans a newly-symlinked episode
before its NFO exists, Jellyfin caches it (title = raw filename, no episode number)
with <lockdata> from the *default* provider, and neither an incremental rescan nor a
forced replaceAllMetadata refresh will ever re-read the NFO afterwards (verified: only
removing + re-adding the item/library fixes an already-stuck item). The metadata fetch
below is the slowest step in the pipeline (network round-trip to GitHub), so op_maint.sh
runs this script in two passes around op_symlink.py: OP_NFO_FETCH_ONLY=1 warms the cache
*before* new symlinks go live, then the normal write pass runs OP_NFO_OFFLINE=1 right
after symlinking so writing each new episode's NFO is pure-filesystem (no network stall)
and the window where a new symlink is visible without a matching NFO shrinks to
milliseconds instead of a multi-second (sometimes much longer, on GitHub hiccups) fetch.
"""
...
LIB = os.environ.get("OP_LIB", "/home/jrbaprz/jellyfin-stack/data/media/anime/One Pace")
# Bundled fallback (checked into git, always present but read-only inside the sidecar
# container since ./onepace is mounted :ro). OP_NFO_CACHE is a *writable* hot cache
# (under /data) that load_meta() refreshes on every successful fetch, so a subsequent
# OP_NFO_OFFLINE=1 run in the same maintenance cycle can reuse today's freshly-fetched
# data instead of falling back to whatever was last committed to git.
META_CACHE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "op_metadata.json")
HOT_CACHE = os.environ.get("OP_NFO_CACHE", "/data/.op_nfo_metadata_cache.json")
...
def load_meta(offline=False):
    if not offline:
        try:
            ...
            for path in (HOT_CACHE, META_CACHE):
                try:                # cache writes are best-effort (META_CACHE's dir may be read-only)
            ...
            print(f"fetch failed ({e}); falling back to cache")
    else:
        print("OP_NFO_OFFLINE=1: skipping live fetch, using cached metadata")
    for path in (HOT_CACHE, META_CACHE):
        try:
            ...
            continue
    raise RuntimeError(f"no metadata available: fetch failed and no cache at {HOT_CACHE} or {META_CACHE}")
...
def main():
    ...
    offline = os.environ.get("OP_NFO_OFFLINE") == "1"
    meta = load_meta(offline=offline)
    if os.environ.get("OP_NFO_FETCH_ONLY") == "1":
        print("OP_NFO_FETCH_ONLY=1: metadata cache warmed, skipping NFO write pass")
        return
